      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
          ...
          <Link href="/consultation">
            <Button variant="outline" className="shrink-0 border-slate-200 text-slate-700 hover:border-blue-300 hover:text-blue-600">
              View all doctors
...
function DoctorCard({ doctor }: { doctor: IDoctor }) {
  ...
  return (
    <Link href={`/consultation/doctor/${doctor.id}`}>
      <div className="group bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg hover:border-blue-200 transition-all duration-200 h-full flex flex-col">
        {/* Top blue band */}
        <div className="h-14 bg-gradient-to-r from-blue-600 to-blue-700 shrink-0" />

        <div className="px-4 pb-4 flex flex-col flex-1 -mt-7">
          {/* Avatar */}
          <div className="relative w-14 h-14 rounded-full overflow-hidden border-[3px] border-white bg-blue-50 shadow mb-3 shrink-0">
            {doctor.profilePhoto ? (
          ...
          {/* Name & designation */}
          <h3 className="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors text-sm leading-tight mb-0.5 truncate">
            {doctor.name}
          </h3>
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {specialties.map((title) => (
                <Badge
                  key={title}
                  variant="secondary"
                  className="text-xs bg-blue-50 text-blue-700 border-0 px-2 py-0.5 font-medium"
                >
                  {title}
                </Badge>
              ))}
          ...
          <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100">
            ...
            <span className="text-xs text-blue-600 font-medium group-hover:underline">Book →</span>
          </div>
        </div>
      </div>
    </Link>
